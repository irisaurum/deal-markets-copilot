...
</div><nav class="mobile-section-nav" aria-label="Разделы" data-ru-aria-label="Разделы" data-en-aria-label="Sections"><span data-ru="Разделы" data-en="Sections">Разделы</span><a href="#overview" data-ru="Обзор" data-en="Overview">Обзор</a><a href="#deals" data-ru="Сделки" data-en="Deals">Сделки</a><a href="#review" data-ru="На проверке" data-en="Review">На проверке</a><a href="#tasks" data-ru="Задачи" data-en="Tasks">Задачи</a><a href="#coverage" data-ru="Источники" data-en="Sources">Источники</a><a href="#downloads" data-ru="Выгрузки" data-en="Exports">Выгрузки</a><a href="#methodology" data-ru="Методология" data-en="Methodology">Методология</a></nav></header>

<main class="page">
  <section class="overview" id="overview">
    <div class="overview-copy"><span class="eyebrow">DEAL MARKETS COPILOT</span><h1 data-ru="Монитор сделок и рынков капитала СНГ" data-en="CIS deal intelligence for M&amp;A, ECM and DCM">Монитор сделок и рынков капитала СНГ</h1>
    <p data-ru="Публичные данные по M&amp;A, ECM и DCM в России и СНГ: первичные источники, статус проверки, качество источников и подтверждений, задачи для аналитика." data-en="Public-source deal data across Russia and CIS markets: primary evidence, review status, source quality and analyst follow-up actions.">Публичные данные по M&amp;A, ECM и DCM в России и СНГ: первичные источники, статус проверки, качество источников и подтверждений, задачи для аналитика.</p>
    ...
    <div class="overview-actions"><a class="button primary" href="#deals" data-ru="Открыть скринер сделок" data-en="Open deal screener">Открыть скринер сделок</a><a class="button ghost" href="precedent_transactions.xlsx" download data-ru="Открыть Excel" data-en="Open Excel">Открыть Excel</a></div></div>
  ...
  <section class="section" id="deals">
    <div class="section-head"><div><span class="eyebrow" data-ru="БАЗА СДЕЛОК" data-en="DEAL DATABASE">БАЗА СДЕЛОК</span><h2 data-ru="Скринер сделок: Россия и СНГ" data-en="Deal screener: Russia and CIS">Скринер сделок: Россия и СНГ</h2><p data-ru="Поиск и фильтрация M&amp;A, ECM и DCM по рынку, качеству источников и подтверждений, стадии и типу источника." data-en="Search and filter M&amp;A, ECM and DCM by market, evidence quality, stage and source type.">Поиск и фильтрация M&amp;A, ECM и DCM по рынку, качеству источников и подтверждений, стадии и типу источника.</p></div>
    ...
    <div class="deal-toolbar"><div class="screener-summary"><b>{len(key_deals)}</b><span data-ru="актуальных сделок в публичном скринере" data-en="current deals in the public screener">актуальных сделок в публичном скринере</span></div><label class="search"><span>⌕</span><input id="deal-search" placeholder="Найти компанию или сделку"></label></div>
    ...
    <details class="data-drawer"><summary><b data-ru="Мультипликаторы прецедентов" data-en="Precedent multiples">Мультипликаторы прецедентов</b><span data-ru="{len(multiple_deals)} сделок · только проверяемые расчёты" data-en="{len(multiple_deals)} deals · verifiable calculations only">{len(multiple_deals)} сделок · только проверяемые расчёты</span></summary>{_precedent_table(multiple_deals[:10])}</details>
  </section>
  ...
  <section class="section analytics" id="tasks">
    <div class="section-head"><div><span class="eyebrow" data-ru="РАБОЧИЙ ПРОЦЕСС АНАЛИТИКА" data-en="ANALYST WORKFLOW">РАБОЧИЙ ПРОЦЕСС АНАЛИТИКА</span><h2 data-ru="Задачи и аналитический вывод" data-en="Analyst tasks and readout">Задачи и аналитический вывод</h2><p data-ru="Задачи создаются только из событий, требующих действий; технические раскрытия не попадают в очередь." data-en="Tasks are created only from actionable events; technical disclosures are excluded from the queue.">Задачи создаются только из событий, требующих действий; технические раскрытия не попадают в очередь.</p></div><button class="button ghost" onclick="window.print()" data-ru="Печать / PDF" data-en="Print / PDF">Печать / PDF</button></div>
...
def _deal_screener_table(rows: list[dict]) -> str:
    ...
    return f'''<div class="screener-table-wrap"><table class="screener-table"><thead><tr><th data-ru="Дата" data-en="Date">Дата</th><th data-ru="Тип" data-en="Type">Тип</th><th data-ru="Эмитент / компания и событие" data-en="Issuer / target and event">Эмитент / компания и событие</th><th data-ru="Рынок" data-en="Market">Рынок</th><th data-ru="Стадия" data-en="Stage">Стадия</th><th data-ru="Сумма" data-en="Value">Сумма</th><th data-ru="Качество" data-en="Quality">Качество</th><th data-ru="Источник" data-en="Source">Источник</th><th data-ru="Подтверждение" data-en="Evidence">Подтверждение</th></tr></thead><tbody>{''.join(body)}</tbody></table><div class="table-filter-empty" hidden data-ru="По выбранным фильтрам сделок нет." data-en="No deals match the selected filters.">По выбранным фильтрам сделок нет.</div></div>'''


def _review_queue_section(buckets: dict[str, list[dict]]) -> str:
    streams = [
        ("watchlist", "На проверке", "Review queue", "Материальные сообщения, которым не хватает первичного подтверждения или обязательных полей.", "Material reports missing primary evidence or required fields."),
        ...
        ("technical_filing", "Технические раскрытия", "Technical disclosures", "Биржевые и регуляторные сообщения сохраняются для аудита, но не создают задачи аналитика.", "Exchange and regulatory notices are retained for traceability but do not create banker tasks."),
    ]
    cards = []
    ...
    return f'''<section class="section" id="review"><div class="section-head"><div><span class="eyebrow" data-ru="ПРОВЕРКА ПОДТВЕРЖДЕНИЙ" data-en="EVIDENCE REVIEW">ПРОВЕРКА ПОДТВЕРЖДЕНИЙ</span><h2 data-ru="Очередь проверки и исключения" data-en="Review queue and exclusions">Очередь проверки и исключения</h2><p data-ru="Статус качества отделён от стадии сделки: «на проверке» означает необходимость ручной верификации, а не отсутствие транзакции. Ниже показана текущая короткая очередь; общий объём записей на проверке в канонической базе указан в верхнем показателе." data-en="Quality status is separate from deal stage: review means manual verification is required, not that no transaction exists. The cards below show the current shortlist; the top KPI reports the full review population in the canonical dataset.">Статус качества отделён от стадии сделки: «на проверке» означает необходимость ручной верификации, а не отсутствие транзакции. Ниже показана текущая короткая очередь; общий объём записей на проверке в канонической базе указан в верхнем показателе.</p></div></div><div class="review-grid">{''.join(cards)}</div></section>'''


def _signals_section(items: list[ClassifiedEvent], new_ids: set[str], counts: Counter, news_window: str) -> str:
    if not items:
        return ""
    news_window_ru = "72 часа" if news_window == "72H CATCH-UP" else "24 часа"
    return f'''<section class="section" id="signals"><div class="section-head"><div><span class="eyebrow" data-ru="ТЕКУЩИЙ ПОТОК СДЕЛОК" data-en="CURRENT DEAL FLOW">ТЕКУЩИЙ ПОТОК СДЕЛОК</span><h2 data-ru="Новые события" data-en="New signals">Новые события</h2><p data-ru="Оперативная лента за {news_window_ru}." data-en="Current feed for {news_window.lower()}.">Оперативная лента за {news_window_ru}.</p></div><div class="filters"><button class="filter active" data-filter="all" data-ru="Все" data-en="All">Все</button><button class="filter" data-filter="new" data-ru="Новые" data-en="New">Новые</button>{_filter_buttons(counts)}</div></div><label class="search event-search"><span>⌕</span><input id="event-search" placeholder="Компания, событие или источник"></label><div class="signal-layout"><div class="event-list">{_event_cards(items, new_ids)}</div><aside class="detail">{_detail_panel(items[0], items[0].event.event_id in new_ids)}</aside></div></section>'''
...
def _status_guide() -> str:
    rows = [
        ("Rumor", "Слух", "Неподтверждённое сообщение рынка", "Unconfirmed market report"),
        ("In talks", "Переговоры", "Стороны обсуждают сделку", "Parties are discussing a transaction"),
        ("Announced", "Объявлено", "Сделка или размещение официально объявлены", "Transaction or issuance formally announced"),
        ("Priced", "Книга закрыта", "Книга заявок закрыта, параметры определены", "Order book closed and terms priced"),
        ("Issued", "Размещено", "Ценные бумаги фактически размещены или выпущены", "Securities actually placed or issued"),
        ("Closed", "Закрыто", "Сделка M&A завершена", "M&A transaction completed"),
        ("Denied", "Опровергнуто", "Сообщение опровергнуто или оспорено", "Report denied or disputed"),
    ]
    cards = "".join(f'<div><b>{html.escape(code)}</b><strong data-ru="{html.escape(ru, quote=True)}" data-en="{html.escape(code, quote=True)}">{html.escape(ru)}</strong><small data-ru="{html.escape(note_ru, quote=True)}" data-en="{html.escape(note_en, quote=True)}">{html.escape(note_ru)}</small></div>' for code, ru, note_ru, note_en in rows)
...
def _source_coverage(registry: list[dict], health: dict | None = None) -> str:
    ru_copy = {
        ...
        "cnpf_moldova": ("Избранные официальные регуляторные события по корпоративным ценным бумагам Молдовы", "Адаптер реализован, но отключён: macOS live-checkpoint прошёл с системным trust store, но CNPF не отдаёт intermediate certificate, а Ubuntu production transport не подтверждён. Охват ограничен allowlist CNPF и не включает BVM."),
    ...
    return f'''<section class="section" id="coverage"><div class="section-head"><div><span class="eyebrow" data-ru="ПОКРЫТИЕ ИСТОЧНИКОВ" data-en="SOURCE COVERAGE">ПОКРЫТИЕ ИСТОЧНИКОВ</span><h2 data-ru="Источники и покрытие рынков СНГ" data-en="CIS sources and market coverage">Источники и покрытие рынков СНГ</h2><p data-ru="Активные источники отделены от дорожной карты. Отложенный источник не считается текущим покрытием." data-en="Active sources are separated from the roadmap. Deferred sources do not count as live coverage.">Активные источники отделены от дорожной карты. Отложенный источник не считается текущим покрытием.</p></div><div class="coverage-summary"><b>{active_count}</b><span data-ru="активных из {len(registry)} отслеживаемых рынков/групп" data-en="active of {len(registry)} tracked markets/groups">активных из {len(registry)} отслеживаемых рынков/групп</span></div></div><div class="coverage-limit" data-ru="Покрытие СНГ расширяется поэтапно: новые рынки подключаются только через официальные источники и проходят проверку качества. В текущей сборке новая запись UZSE была найдена, но не добавлена в базу из-за архивного окна." data-en="CIS coverage is expanding gradually. New markets are added only through official sources and quality gates. In this build, a UZSE disclosure was detected but excluded by the archive window.">Покрытие СНГ расширяется поэтапно: новые рынки подключаются только через официальные источники и проходят проверку качества. В текущей сборке новая запись UZSE была найдена, но не добавлена в базу из-за архивного окна.</div><div class="coverage-grid">{''.join(rows)}</div></section>'''


def _downloads_integrity(health: dict) -> str:
    return '''<section class="section downloads" id="downloads"><div class="section-head"><div><span class="eyebrow" data-ru="ВЫГРУЗКИ" data-en="EXPORTS">ВЫГРУЗКИ</span><h2 data-ru="Выгрузки для аналитика" data-en="Analyst exports">Выгрузки для аналитика</h2><p data-ru="Единая база в трёх форматах: рабочая книга, плоская таблица и манифест сборки." data-en="One dataset in three formats: analyst workbook, flat file and build manifest.">Единая база в трёх форматах: рабочая книга, плоская таблица и манифест сборки.</p></div></div><div class="download-grid"><a href="precedent_transactions.xlsx" download><b>XLSX</b><span data-ru="Пятилистная рабочая книга: Сводка, Сделки, Финансовые показатели, Мультипликаторы, Источники и проверка" data-en="Five-sheet workbook: Summary, Deals, Financials, Multiples, Sources &amp; QA">Пятилистная рабочая книга: Сводка, Сделки, Финансовые показатели, Мультипликаторы, Источники и проверка</span></a><a href="precedent_transactions.csv" download><b>CSV</b><span data-ru="Машиночитаемая база сделок" data-en="Machine-readable deal database">Машиночитаемая база сделок</span></a><a href="build_manifest.json"><b>JSON</b><span data-ru="Идентификатор сборки, SHA базы и число записей" data-en="Build ID, dataset SHA and record count">Идентификатор сборки, SHA базы и число записей</span></a></div></section>'''
...
def _methodology_integrity(health: dict) -> str:
    ...
    return f'''<section class="section methodology" id="methodology"><div class="section-head"><div><span class="eyebrow" data-ru="МЕТОДОЛОГИЯ И ЦЕЛОСТНОСТЬ" data-en="METHODOLOGY &amp; INTEGRITY">МЕТОДОЛОГИЯ И ЦЕЛОСТНОСТЬ</span><h2 data-ru="Как формируется база" data-en="How the database is built">Как формируется база</h2><p data-ru="Публичное событие проходит дедупликацию, классификацию, нормализацию, проверку подтверждений и проверку качества до попадания в скринер." data-en="A public event passes deduplication, classification, normalization, evidence review and a quality gate before entering the screener.">Публичное событие проходит дедупликацию, классификацию, нормализацию, проверку подтверждений и проверку качества до попадания в скринер.</p></div></div><div class="method-flow"><span data-ru="Публичные источники" data-en="Public sources">Публичные источники</span><i>→</i><span data-ru="Классификация и фильтр шума" data-en="Classification and noise filter">Классификация и фильтр шума</span><i>→</i><span data-ru="Нормализация сделки" data-en="Deal normalization">Нормализация сделки</span><i>→</i><span data-ru="Проверка качества" data-en="Quality gate">Проверка качества</span><i>→</i><span data-ru="Скринер и рабочий процесс" data-en="Screener and workflow">Скринер и рабочий процесс</span></div><div class="integrity-grid"><div><span>Build ID</span><b>{build_id}</b></div><div><span>Dataset SHA-256</span><b>{dataset_sha}</b></div><div><span data-ru="Записей" data-en="Records">Записей</span><b>{records}</b></div><div><span data-ru="Критические ошибки проверки" data-en="Critical QA issues">Критические ошибки проверки</span><b>{issues}</b></div><div><span>Excel</span><b data-ru="{'синхронизирован' if synced else 'ожидает сборки'}" data-en="{'synchronized' if synced else 'awaiting build'}">{'синхронизирован' if synced else 'ожидает сборки'}</b></div></div><div class="method-note" data-ru="Инструмент скрининга публичных данных: не заменяет Bloomberg, Dealogic, Capital IQ, LSEG или ручную проверку документов по сделкам." data-en="Public-data screening tool: not a replacement for Bloomberg, Dealogic, Capital IQ, LSEG or manual review of transaction documents.">Инструмент скрининга публичных данных: не заменяет Bloomberg, Dealogic, Capital IQ, LSEG или ручную проверку документов по сделкам.</div></section>'''


def _duration_label(minutes: float, lang: str) -> str:
...
def _freshness_labels(health: dict) -> tuple[str, str, str]:
    age = health.get("source_age_minutes")
    limit = health.get("freshness_limit_minutes")
    if not isinstance(age, (int, float)) or isinstance(age, bool):
        return "требуется обновление", "refresh required", "stale"
    limit_minutes = float(limit) if isinstance(limit, (int, float)) and not isinstance(limit, bool) else 90.0
    stale = health.get("freshness_status") != "ok" or float(age) > limit_minutes
    if stale:
        return "требуется обновление", "refresh required", "stale"
    if float(age) <= min(90.0, limit_minutes):
        return "свежие данные", "fresh data", "fresh"
    return (
        f"Последняя проверка: {_duration_label(float(age), 'ru')} назад",
        f"Last checked {_duration_label(float(age), 'en')} ago",
        "checked",
    )
...
def _health_panel(health: dict) -> str:
    ...
    freshness, freshness_en, freshness_state = _freshness_labels(health)
    age = health.get("source_age_minutes")
    age_attr = f"{float(age):.1f}" if isinstance(age, (int, float)) and not isinstance(age, bool) else ""
    freshness_limit = health.get("freshness_limit_minutes")
    limit_attr = f"{float(freshness_limit):.1f}" if isinstance(freshness_limit, (int, float)) and not isinstance(freshness_limit, bool) else "90"
    ...
    market_label_en = {
        "ok": f"OK · {market_count}/{market_total}",
        "partial": f"partial · {market_count}/{market_total}",
        "error": "source error",
        "unavailable": f"unavailable · {market_count}/{market_total}",
    }.get(str(market_status), "not checked")
    return f'''<section class="health-panel {css}" id="data-health" data-last-success="{updated}" data-source-age-minutes="{age_attr}" data-freshness-limit-minutes="{limit_attr}" data-freshness-status="{html.escape(str(health.get('freshness_status') or ''), quote=True)}" aria-label="Состояние системы" data-ru-aria-label="Состояние системы" data-en-aria-label="System status">
      ...
      <dl><div><dt>Build ID</dt><dd>{build_id}</dd></div><div><dt data-ru="Записей" data-en="Records">Записей</dt><dd>{records}</dd></div><div><dt data-ru="Подтверждено" data-en="Approved">Подтверждено</dt><dd>{approved}</dd></div><div><dt data-ru="Источники" data-en="Sources">Источники</dt><dd data-ru="{source_status}" data-en="{source_status_en}">{source_status}</dd></div><div><dt data-ru="Рыночный контекст" data-en="Market tape">Рыночный контекст</dt><dd data-ru="{market_label}" data-en="{market_label_en}">{market_label}</dd></div><div><dt data-ru="Давность данных" data-en="Data freshness">Давность данных</dt><dd id="data-freshness-value" class="freshness-value {freshness_state}" data-ru="{freshness}" data-en="{freshness_en}">{freshness}</dd></div><div><dt data-ru="Ошибки проверки" data-en="QA issues">Ошибки проверки</dt><dd>{issues}</dd></div><div><dt>Excel</dt><dd data-ru="{'синхронизирован' if synced else 'ожидает сборки'}" data-en="{'synchronized' if synced else 'awaiting build'}">{'синхронизирован' if synced else 'ожидает сборки'}</dd></div></dl>
    </section>'''
...
def _deal_bucket_panels(buckets: dict[str, list[dict]]) -> str:
    metadata = {
        ...
        "technical_filing": ("Технические раскрытия", "Technical filings", "Официальные документы биржи: параметры выпуска без смешивания со сделками.", "Official exchange documents kept separate from transaction flow."),
    }
    panels = []
...
def _typed_deal_body(row: dict) -> str:
    ...
    if deal_type == "M&A":
        entities = _entity_grid([
            ...
            ("Компания / цель сделки", row.get("target_or_issuer")),
        ...
        facts = [
            ...
            ("Акции в свободном обращении", _display_percent(row.get("free_float_percent"))),
        ]
...
def _typed_deal_details(row: dict) -> str:
    ...
    if deal_type == "M&A":
        fields = [
            ("Консультанты", row.get("advisors")), ("Обоснование", row.get("rationale")),
            ("EV", _deal_amount(row.get("enterprise_value"), row.get("currency", ""))),
            ("Выручка LTM", _deal_amount(row.get("revenue_ltm"), row.get("financials_currency", ""))),
            ("EBITDA LTM", _deal_amount(row.get("ebitda_ltm"), row.get("financials_currency", ""))),
            ("Финансовые данные на", row.get("financials_as_of")),
            ("Источник финансовых данных", row.get("financials_source_name")),
            ("Комментарий к мультипликатору", row.get("multiple_notes")),
        ]
    elif deal_type == "DCM":
        ...
        fields = [("Организаторы", row.get("bookrunners")), ("Обоснование", row.get("rationale")), ("Валюта", row.get("currency"))]
...
def _en_label(value: str) -> str:
    return {
        ...
        "Компания / цель сделки": "Target", "Акции в свободном обращении": "Free float",
        "Консультанты": "Advisors", "Обоснование": "Rationale", "Выручка LTM": "Revenue LTM",
        "Финансовые данные на": "Financials as of", "Источник финансовых данных": "Financial source",
        "Комментарий к мультипликатору": "Multiple note", "Организаторы": "Bookrunners",
    }.get(str(value), str(value))
...
def _quality_note(row: dict) -> str:
    ...
    labels = {
        ...
        "missing_target": "не определена целевая компания",
        ...
        "single_secondary_source": "нужен первичный источник или независимое подтверждение",
        "price_target_context": "обнаружен контекст целевой цены",
        ...
        "technical_filing": "официальное раскрытие, не сделка",
...
def _precedent_table(rows: list[dict]) -> str:
    ...
    output = []
    for row in rows:
        output.append(f"""<tr><td>{html.escape(row.get('announced_date','—') or '—')}</td><td><b>{html.escape(row.get('deal_type',''))}</b><small data-ru="{html.escape(_status_label(row.get('status','Rumor')), quote=True)}" data-en="{html.escape(_status_label_en(row.get('status','Rumor')), quote=True)}">{html.escape(_status_label(row.get('status','Rumor')))}</small></td>
        <td class="deal-headline"><b>{html.escape(row.get('headline','Not disclosed'))}</b></td>
        <td data-ru="{html.escape(_display_value(row.get('target_or_issuer')), quote=True)}" data-en="{html.escape(_display_value_en(row.get('target_or_issuer')), quote=True)}">{html.escape(_display_value(row.get('target_or_issuer')))}</td><td data-ru="{html.escape(_display_value(row.get('acquirer_or_investor')), quote=True)}" data-en="{html.escape(_display_value_en(row.get('acquirer_or_investor')), quote=True)}">{html.escape(_display_value(row.get('acquirer_or_investor')))}</td>
        <td data-ru="{html.escape(_display_amount(row), quote=True)}" data-en="{html.escape(_deal_amount(row.get('transaction_value'), row.get('currency','')), quote=True)}">{html.escape(_display_amount(row))}</td><td>{html.escape(_percent(row.get('stake_percent')))}</td>
        <td>{html.escape(_multiple(row.get('ev_revenue')))}</td><td>{html.escape(_multiple(row.get('ev_ebitda')))}</td><td data-ru="{html.escape(_display_value(row.get('payment_form')), quote=True)}" data-en="{html.escape(_display_value_en(row.get('payment_form')), quote=True)}">{html.escape(_display_value(row.get('payment_form')))}</td>
        <td><span class="quality-badge {html.escape(row.get('quality_status','review'))}" data-ru="{html.escape(_quality_label(row.get('quality_status','review')), quote=True)}" data-en="{html.escape(_quality_label_en(row.get('quality_status','review')), quote=True)}">{html.escape(_quality_label(row.get('quality_status','review')))}</span><small>{int(row.get('quality_score') or 0)}/100</small>{_quality_note(row)}</td>
        <td><div class="source-list table-sources">{_deal_source_links(row)}</div></td></tr>""")
    return f'<div class="table-wrap"><table><thead><tr><th data-ru="Дата" data-en="Date">Дата</th><th data-ru="Тип" data-en="Type">Тип</th><th data-ru="Сделка / событие" data-en="Deal / event">Сделка / событие</th><th data-ru="Компания / эмитент" data-en="Target / issuer">Компания / эмитент</th><th data-ru="Покупатель" data-en="Acquirer">Покупатель</th><th data-ru="Стоимость" data-en="Value">Стоимость</th><th data-ru="Доля" data-en="Stake">Доля</th><th>EV/Revenue</th><th>EV/EBITDA</th><th data-ru="Форма оплаты" data-en="Payment">Форма оплаты</th><th data-ru="Качество" data-en="Quality">Качество</th><th data-ru="Источники" data-en="Sources">Источники</th></tr></thead><tbody>{"".join(output)}</tbody></table></div>'
...
def _task_rows(tasks: list[dict]) -> str:
    ...
    for task in tasks:
        ...
        <small>{html.escape(task.get('coverage',''))} · {html.escape(hypotheses)} · <span data-ru="РЕЗУЛЬТАТ" data-en="DELIVERABLE">РЕЗУЛЬТАТ</span>: {html.escape(task.get('deliverable',''))}</small>{context_html}</span>
        <span class="state {html.escape(state)}" data-ru="{'ОТКРЫТО' if state == 'open' else html.escape(state.upper(), quote=True)}" data-en="{html.escape(state.upper(), quote=True)}">{'ОТКРЫТО' if state == 'open' else html.escape(state.upper())}</span>
...
def _hypothesis_cards(hypotheses: list[dict]) -> str:
    ...
    for hypothesis in hypotheses:
        ...
        <div class="hypothesis-top"><span>{html.escape(hypothesis.get('id',''))} · {html.escape(hypothesis.get('stage',''))}</span><b data-ru="{'НА ПРОВЕРКЕ' if status == 'attention' else 'АКТИВНО'}" data-en="{'REVIEW' if status == 'attention' else 'ACTIVE'}">{'НА ПРОВЕРКЕ' if status == 'attention' else 'АКТИВНО'}</b></div>
        <h3>{html.escape(hypothesis.get('title',''))}</h3><p>{html.escape(hypothesis.get('thesis',''))}</p>
        <dl><div><dt data-ru="Вопрос для решения" data-en="Decision question">Вопрос для решения</dt><dd>{html.escape(hypothesis.get('decision_question',''))}</dd></div>
        <div><dt data-ru="Следующий результат" data-en="Next deliverable">Следующий результат</dt><dd>{html.escape(hypothesis.get('deliverable',''))}</dd></div></dl>
        <div class="hypothesis-foot"><span data-ru="{hypothesis.get('signal_count',0)} сигналов · {hypothesis.get('new_signal_count',0)} новых" data-en="{hypothesis.get('signal_count',0)} signals · {hypothesis.get('new_signal_count',0)} new">{hypothesis.get('signal_count',0)} сигналов · <b>{hypothesis.get('new_signal_count',0)} новых</b></span><span>{html.escape(' · '.join(hypothesis.get('tickers',[])))}</span></div></article>""")
    return "".join(cards)
...
def _event_cards(items: list[ClassifiedEvent], new_ids: set[str]) -> str:
    ...
    for index, item in enumerate(items):
        ...
        cards.append(f"""<article class="event-card{' selected' if index == 0 else ''}" data-index="{index}" data-category="{html.escape(item.category, quote=True)}" data-new="{'true' if is_new else 'false'}" data-search="{html.escape((event.title+' '+event.source+' '+coverage).lower(), quote=True)}">
          ...
          <div class="event-source">{html.escape(event.source)} · <span data-ru="{html.escape(_evidence_label(item.evidence_label), quote=True)}" data-en="{html.escape(_evidence_label_en(item.evidence_label), quote=True)}">{html.escape(_evidence_label(item.evidence_label))}</span></div></div>
...
def _detail_panel(item: ClassifiedEvent | None, is_new: bool) -> str:
    ...
    return f"""<span class="kicker" data-ru="КРАТКАЯ ЗАПИСКА" data-en="BANKER BRIEF">КРАТКАЯ ЗАПИСКА</span><div class="detail-score"><span id="detail-score">{item.score}/10</span><small id="detail-state">{'NEW SIGNAL' if is_new else item.severity.upper()}</small></div>
    ...
    <div class="detail-grid"><div><span data-ru="ТИП" data-en="TYPE">ТИП</span><strong id="detail-category">{html.escape(payload['category'])}</strong></div>
    <div><span data-ru="ПОКРЫТИЕ" data-en="COVERAGE">ПОКРЫТИЕ</span><strong id="detail-coverage">{html.escape(payload['coverage'])}</strong></div>
    <div><span data-ru="ПОДТВЕРЖДЕНИЕ" data-en="EVIDENCE">ПОДТВЕРЖДЕНИЕ</span><strong id="detail-evidence" data-ru="{html.escape(payload['evidence'], quote=True)}" data-en="{html.escape(payload['evidence_en'], quote=True)}">{html.escape(payload['evidence'])}</strong></div>
    <div><span data-ru="ИСТОЧНИК" data-en="SOURCE">ИСТОЧНИК</span><strong id="detail-source">{html.escape(payload['source'])}</strong></div></div>
    <a id="detail-link" class="primary-link" href="{html.escape(payload['url'], quote=True)}" target="_blank" rel="noopener" data-ru="{html.escape(payload['proof_label'], quote=True)}" data-en="{html.escape(payload['proof_label_en'], quote=True)}">{html.escape(payload['proof_label'])}</a>"""
...
def _quote_table(quotes: list[dict]) -> str:
    ...
    return f'<div class="table-wrap"><table><thead><tr><th data-ru="Бумага" data-en="Security">Бумага</th><th data-ru="Последняя цена" data-en="Last">Последняя цена</th><th data-ru="Изменение" data-en="Change">Изменение</th><th data-ru="Оборот" data-en="Turnover">Оборот</th><th data-ru="Обновлено" data-en="Updated">Обновлено</th><th data-ru="Источник" data-en="Source">Источник</th></tr></thead><tbody>{"".join(rows)}</tbody></table></div>'
...
def _source_table(items: list[ClassifiedEvent]) -> str:
    ...
    return f'<div class="table-wrap"><table><thead><tr><th>ID</th><th data-ru="Источник" data-en="Source">Источник</th><th data-ru="Дата" data-en="Date">Дата</th><th data-ru="Статус" data-en="Status">Статус</th><th data-ru="Тип" data-en="Type">Тип</th><th data-ru="Сообщение" data-en="Claim">Сообщение</th><th data-ru="Ссылка" data-en="Link">Ссылка</th></tr></thead><tbody>{"".join(rows)}</tbody></table></div>'
...
def _event_payload(item: ClassifiedEvent, is_new: bool) -> dict:
    return {
        ...
        "coverage": ", ".join(item.matched_coverage) or "MARKET",
        "evidence": _evidence_label(item.evidence_label), "evidence_en": _evidence_label_en(item.evidence_label), "source": item.event.source,
        ...
        "proof_label": _proof_label(item.event.url), "proof_label_en": _proof_label_en(item.event.url),
    }
...
def _proof_label_en(value: str) -> str:
    return "Open via Google News ↗" if "news.google.com" in str(value) else "Evidence ↗"


def _evidence_label(value: str) -> str:
    return {"confirmed": "Подтверждено", "unverified": "Не проверено"}.get(str(value), str(value))


def _evidence_label_en(value: str) -> str:
    return {"confirmed": "Confirmed", "unverified": "Unverified"}.get(str(value), str(value))
...
function setLanguage(lang){document.documentElement.lang=lang;document.querySelectorAll('[data-ru][data-en]').forEach(node=>{node.textContent=node.dataset[lang];});document.querySelectorAll('[data-ru-aria-label][data-en-aria-label]').forEach(node=>node.setAttribute('aria-label',node.dataset[lang+'AriaLabel']));langButtons.forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.lang===lang)));const placeholders={ru:{'deal-search':'Найти компанию или сделку','event-search':'Компания, событие или источник'},en:{'deal-search':'Find a company or deal','event-search':'Company, event or source'}};Object.entries(placeholders[lang]).forEach(([id,value])=>{const node=document.getElementById(id);if(node)node.placeholder=value;});localStorage.setItem('dealDeskLanguage',lang);}
...
document.getElementById('detail-title').textContent=d.title;document.getElementById('detail-angle').textContent=d.banker_angle;document.getElementById('detail-action').textContent=d.next_action;document.getElementById('detail-category').textContent=d.category;document.getElementById('detail-coverage').textContent=d.coverage;const evidence=document.getElementById('detail-evidence');evidence.dataset.ru=d.evidence;evidence.dataset.en=d.evidence_en;evidence.textContent=document.documentElement.lang==='en'?d.evidence_en:d.evidence;document.getElementById('detail-source').textContent=d.source;document.getElementById('detail-score').textContent=d.score+'/10';document.getElementById('detail-state').textContent=d.is_new?'NEW SIGNAL':d.severity.toUpperCase();const detailLink=document.getElementById('detail-link');detailLink.href=d.url;detailLink.dataset.ru=d.proof_label;detailLink.dataset.en=d.proof_label_en;detailLink.textContent=document.documentElement.lang==='en'?d.proof_label_en:d.proof_label;}));
...
function freshnessDuration(minutes,lang){const total=Math.max(0,Math.round(minutes)),hours=Math.floor(total/60),remaining=total%60;if(lang==='ru'){if(hours&&remaining)return `${hours} ч ${remaining} мин`;if(hours)return `${hours} ч`;return `${remaining} мин`;}if(hours&&remaining)return `${hours}h ${remaining}m`;if(hours)return `${hours}h`;return `${remaining}m`;}
const healthPanel=document.getElementById('data-health');
function updateFreshness(){if(!healthPanel)return;const value=document.getElementById('data-freshness-value');if(!value)return;const last=Date.parse(healthPanel.dataset.lastSuccess),fallback=Number(healthPanel.dataset.sourceAgeMinutes),age=Number.isFinite(last)?Math.max(0,(Date.now()-last)/60000):fallback,now=new Date(),hour=Number(new Intl.DateTimeFormat('en-GB',{timeZone:'Europe/Moscow',hour:'2-digit',hour12:false}).format(now)),weekday=new Intl.DateTimeFormat('en-US',{timeZone:'Europe/Moscow',weekday:'short'}).format(now),working=!['Sat','Sun'].includes(weekday)&&hour>=8&&hour<20,maxAge=working?90:72*60,serverStale=healthPanel.dataset.freshnessStatus==='stale';let state='stale',ru='требуется обновление',en='refresh required';if(Number.isFinite(age)&&!serverStale&&age<=maxAge){if(age<=90){state='fresh';ru='свежие данные';en='fresh data';}else{state='checked';ru=`Последняя проверка: ${freshnessDuration(age,'ru')} назад`;en=`Last checked ${freshnessDuration(age,'en')} ago`;}}value.dataset.ru=ru;value.dataset.en=en;value.className=`freshness-value ${state}`;value.textContent=document.documentElement.lang==='en'?en:ru;}
updateFreshness();setInterval(updateFreshness,60*1000);
...
@media(max-width:980px){html{scroll-padding-top:126px}.site-header .desktop-nav{display:none}.mobile-section-nav{display:flex;align-items:center;gap:6px;width:100%;padding:7px 16px 9px;border-top:1px solid var(--line);overflow-x:auto;overscroll-behavior-x:contain;scrollbar-width:none;white-space:nowrap}.mobile-section-nav::-webkit-scrollbar{display:none}.mobile-section-nav span,.mobile-section-nav a{flex:none;padding:5px 9px;border-radius:8px;font-size:10px;font-weight:700;text-decoration:none}.mobile-section-nav span{background:var(--ink);color:#fff}.mobile-section-nav a{background:var(--surface-soft);color:var(--muted)}.mobile-section-nav a:hover,.mobile-section-nav a:focus-visible{background:var(--blue-soft);color:var(--blue)}.overview{grid-template-columns:1fr;padding:36px}.overview-side{grid-template-columns:1fr 1fr;align-items:start}.metric-grid{grid-template-columns:repeat(4,1fr)}.health-panel{align-items:flex-start;flex-direction:column}.health-panel dl{width:100%;flex-wrap:wrap}.deal-toolbar{flex-direction:column}.deal-advanced-filters{grid-template-columns:repeat(3,1fr)}.deal-grid,.analytics-grid,.utility-grid{grid-template-columns:1fr}.signal-layout{grid-template-columns:1fr}.detail{position:static}.market-strip{grid-template-columns:1fr}.quote-card{border-right:0;border-bottom:1px solid var(--line)}.quote-card:last-child{border-bottom:0}}
...
.status-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;padding:15px;border-top:1px solid var(--line)}.status-grid div{display:grid;gap:3px;padding:11px;border-radius:9px;background:var(--surface-soft)}.status-grid b{color:var(--blue);font-size:9px}.status-grid strong{font-size:11px}.status-grid small{color:var(--muted);font-size:9px}.coverage-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.coverage-card{display:flex;flex-direction:column;min-width:0;min-height:250px;padding:18px;border:1px solid var(--line);border-radius:13px;background:var(--surface)}.coverage-card.active{border-color:#a8d8c8}.coverage-card>div:first-child{display:flex;justify-content:space-between;color:var(--muted);font-size:9px;text-transform:uppercase}.coverage-card>div:first-child b{color:var(--amber)}.coverage-card.active>div:first-child b{color:var(--mint)}.coverage-card h3{margin:13px 0 5px;font-size:15px}.coverage-card p,.coverage-card>small{color:var(--muted);font-size:10px}.coverage-card dl{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.coverage-card dt{color:var(--muted);font-size:8px;text-transform:uppercase}.coverage-card dd{margin:2px 0;font-size:10px}.coverage-card h3,.coverage-card p,.coverage-card dd,.coverage-card>small,.coverage-card>a{overflow-wrap:anywhere}.coverage-card>a{margin-top:auto;color:var(--blue);font-size:10px;font-weight:750;text-decoration:none}.download-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.download-grid>a,.download-grid>div{display:grid;gap:5px;padding:16px;border:1px solid var(--line);border-radius:11px;text-decoration:none;background:var(--surface-soft)}.download-grid b{color:var(--blue)}.download-grid span{color:var(--muted);font-size:10px;overflow-wrap:anywhere}
@media(max-width:980px){.coverage-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.download-grid,.status-grid{grid-template-columns:1fr 1fr}.deal-advanced-filters{grid-template-columns:repeat(3,1fr)}}
@media(max-width:680px){.nav-wrap{gap:10px}.lang-toggle{margin-left:auto}.status{margin-left:0}.coverage-grid{grid-template-columns:minmax(0,1fr)}.download-grid,.status-grid{grid-template-columns:1fr}.deal-advanced-filters{grid-template-columns:1fr 1fr}}
.nav-wrap{gap:18px}.brand{min-width:205px}.brand-mark{width:35px;height:35px;border-radius:9px}.brand strong{font-size:13px}.brand small{font-size:8px}.site-header .desktop-nav{gap:14px;flex:1;justify-content:center}.site-header .desktop-nav a{font-size:11px;white-space:nowrap}.status{margin-left:0}.status strong{font-size:9px}.status small{font-size:9px}
.overview{grid-template-columns:minmax(0,1.15fr) minmax(480px,.85fr);gap:34px;padding:30px 34px}.overview h1{font-size:30px;line-height:1.14;letter-spacing:-.035em;margin:0 0 12px;max-width:660px;color:#fff}.overview-copy>p{font-size:13px;line-height:1.55}.hero-disclaimer{display:block;color:#9eabc2;font-size:9px;margin-top:8px}.overview-actions{margin-top:18px}.product-kpis{grid-template-columns:repeat(5,minmax(0,1fr));align-self:center;gap:7px}.product-kpis article{padding:13px 11px;min-height:98px}.product-kpis span,.product-kpis small{font-size:8px}.product-kpis strong{font-size:24px;margin:7px 0 5px}
