...
def build_html_report(
    ...
    multiple_deals = sorted(
        (row for row in precedents if row.get("deal_type") == "M&A" and (row.get("ev_revenue") or row.get("ev_ebitda"))),
...
<main class="page">
  ...
  <section class="section" id="deals">
    ...
    <div class="deal-advanced-filters" aria-label="Фильтры сделок">
      ...
      <label>Размер<select id="deal-size-filter"><option value="all">Любой</option><option value="disclosed">Сумма раскрыта</option><option value="undisclosed">Не раскрыта</option><option value="large">≥ 10 млрд в валюте сделки</option></select></label>
      <label>Сортировка<select id="deal-sort"><option value="date-desc">Сначала новые</option><option value="date-asc">Сначала старые</option><option value="amount-desc">По сумме ↓</option><option value="score-desc">По качеству ↓</option></select></label>
...
def _deal_tiles(rows: list[dict], bucket: str = "deal") -> str:
    ...
    for row in rows:
        ...
        cards.append(f"""<article class="deal-tile" data-deal-bucket="{html.escape(bucket, quote=True)}" data-deal-type="{html.escape(deal_type, quote=True)}" data-deal-sector="{html.escape(str(row.get('sector') or 'Not classified'), quote=True)}" data-deal-status="{html.escape(str(row.get('status') or ''), quote=True)}" data-deal-date="{html.escape(str(row.get('announced_date') or ''), quote=True)}" data-deal-amount="{html.escape(str(row.get('transaction_value') or ''), quote=True)}" data-deal-score="{int(row.get('quality_score') or 0)}" data-deal-search="{html.escape(search, quote=True)}">
...
@media(max-width:980px){.site-header nav{display:none}.overview{grid-template-columns:1fr;padding:36px}.overview-side{grid-template-columns:1fr 1fr;align-items:start}.metric-grid{grid-template-columns:repeat(4,1fr)}.deal-toolbar{flex-direction:column}.deal-advanced-filters{grid-template-columns:repeat(3,1fr)}.deal-grid,.analytics-grid,.utility-grid{grid-template-columns:1fr}.signal-layout{grid-template-columns:1fr}.detail{position:static}.market-strip{grid-template-columns:1fr}.quote-card{border-right:0;border-bottom:1px solid var(--line)}.quote-card:last-child{border-bottom:0}}
