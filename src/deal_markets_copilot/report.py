...
<main class="page">
  ...
  <section class="section" id="deals">
    ...
    <div class="deal-toolbar"><div class="deal-filters"><button class="deal-filter active" data-deal-filter="all">Все <span>{len(key_deals)}</span></button><button class="deal-filter" data-deal-filter="M&A">M&amp;A</button><button class="deal-filter" data-deal-filter="ECM">ECM</button><button class="deal-filter" data-deal-filter="DCM">DCM</button></div>
    <label class="search"><span>⌕</span><input id="deal-search" placeholder="Найти компанию или сделку"></label></div>
    <div class="deal-stats"><span><b>{len(precedents)}</b> записей в архиве</span><span>Медиана EV/Revenue <b>{_multiple(precedent_stats.get('ev_revenue'))}</b></span><span>Медиана EV/EBITDA <b>{_multiple(precedent_stats.get('ev_ebitda'))}</b></span></div>
    <div class="deal-grid">{_deal_tiles(key_deals)}</div>
...
def _deal_tiles(rows: list[dict]) -> str:
    if not rows:
        return '<div class="empty">Сделки появятся после следующего обновления.</div>'
    cards = []
    for row in rows:
        deal_type = row.get("deal_type", "Other")
        target = row.get("target_or_issuer", "Not disclosed")
        acquirer = row.get("acquirer_or_investor", "Not disclosed")
        target_label = "Не раскрыто" if target in {"", "Not disclosed"} else target
        acquirer_label = "Не применимо" if acquirer == "Not applicable" else "Не раскрыто" if acquirer in {"", "Not disclosed"} else acquirer
        amount = _deal_amount(row.get("transaction_value"), row.get("currency", ""))
        amount = "Не раскрыта" if amount == "Not disclosed" else amount.replace("bn", "млрд").replace("mm", "млн")
        stake = _percent(row.get("stake_percent"))
        stake = "Не раскрыта" if stake == "Not disclosed" else stake
        source_url = _safe_url(row.get("source_url", ""))
        search = " ".join(str(row.get(key, "")) for key in ("headline", "target_or_issuer", "acquirer_or_investor", "source_name")).lower()
        cards.append(f"""<article class="deal-tile" data-deal-type="{html.escape(deal_type, quote=True)}" data-deal-search="{html.escape(search, quote=True)}">
          <div class="deal-tile-top"><span class="type-badge type-{html.escape(deal_type.lower().replace('&','a'), quote=True)}">{html.escape(deal_type)}</span><span class="deal-status">{html.escape(row.get('status','Reported'))}</span><time>{html.escape(row.get('announced_date','—') or '—')}</time></div>
          <h3>{html.escape(row.get('headline','Сделка без заголовка'))}</h3>
          <div class="parties"><div><span>Компания / актив</span><strong>{html.escape(target_label)}</strong></div><span class="party-arrow">→</span><div><span>Покупатель / инвестор</span><strong>{html.escape(acquirer_label)}</strong></div></div>
          <div class="deal-facts"><div><span>Сумма</span><strong>{html.escape(amount)}</strong></div><div><span>Доля</span><strong>{html.escape(stake)}</strong></div><div><span>EV/EBITDA</span><strong>{html.escape(_multiple(row.get('ev_ebitda')))}</strong></div></div>
          <div class="deal-tile-foot"><span>{html.escape(row.get('source_name','Источник'))}</span><a href="{html.escape(source_url, quote=True)}" target="_blank" rel="noopener">{_proof_label(source_url)}</a></div>
        </article>""")
    return "".join(cards)
...
const dealCards=[...document.querySelectorAll('.deal-tile')],dealFilters=[...document.querySelectorAll('.deal-filter')],dealSearch=document.getElementById('deal-search');let activeDeal='all';
function applyDealFilters(){const q=dealSearch.value.trim().toLowerCase();dealCards.forEach(card=>{const typeOk=activeDeal==='all'||card.dataset.dealType===activeDeal;const searchOk=!q||card.dataset.dealSearch.includes(q);card.hidden=!(typeOk&&searchOk);});}
dealFilters.forEach(button=>button.addEventListener('click',()=>{dealFilters.forEach(x=>x.classList.remove('active'));button.classList.add('active');activeDeal=button.dataset.dealFilter;applyDealFilters();}));dealSearch.addEventListener('input',applyDealFilters);
...
.market-strip{display:grid;grid-template-columns:repeat(3,1fr);margin:18px 0 26px;background:var(--surface);border:1px solid var(--line);border-radius:14px;overflow:hidden}.quote-card{display:grid;grid-template-columns:1fr auto auto;gap:14px;align-items:center;padding:13px 17px;border-right:1px solid var(--line);text-decoration:none}.quote-card:last-child{border:0}.quote-card strong,.quote-card small{display:block}.quote-card small{color:var(--muted);font-size:10px}.quote-price,.quote-change{font-weight:750;font-variant-numeric:tabular-nums}.quote-change{font-size:12px}.up,.positive{color:var(--mint)}.down,.negative{color:var(--red)}.flat{color:var(--muted)}.tape-empty{padding:15px;color:var(--muted);grid-column:1/-1}
.section{margin-top:24px;padding:30px;background:var(--surface);border:1px solid var(--line);border-radius:18px;box-shadow:0 5px 18px rgba(31,45,72,.035)}.section-head{display:flex;justify-content:space-between;align-items:flex-start;gap:24px;margin-bottom:22px}.section-head h2{font-size:27px;line-height:1.15;letter-spacing:-.025em;margin:0}.section-head p{color:var(--muted);margin:8px 0 0;max-width:700px}.export-actions,.filters,.deal-filters{display:flex;gap:8px;flex-wrap:wrap}.deal-toolbar{display:flex;justify-content:space-between;gap:14px;margin-bottom:14px}.deal-filter,.filter{border:1px solid var(--line);background:var(--surface);color:var(--muted);border-radius:9px;padding:8px 12px;font-size:12px;font-weight:700;cursor:pointer}.deal-filter:hover,.deal-filter.active,.filter:hover,.filter.active{background:var(--blue-soft);border-color:#aebcf2;color:var(--blue-dark)}.deal-filter span{margin-left:4px;color:var(--blue)}.search{display:flex;align-items:center;gap:8px;border:1px solid var(--line);background:var(--surface-soft);border-radius:10px;padding:8px 12px;min-width:290px}.search span{color:var(--blue);font-size:17px}.search input{width:100%;border:0;outline:0;background:transparent;color:var(--ink);font-size:12px}.search input::placeholder{color:#98a0ae}.deal-stats{display:flex;gap:24px;flex-wrap:wrap;padding:12px 0 18px;color:var(--muted);font-size:11px}.deal-stats b{color:var(--ink)}
.deal-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.deal-tile{display:flex;flex-direction:column;min-height:300px;border:1px solid var(--line);border-radius:15px;padding:20px;background:var(--surface);transition:.18s ease}.deal-tile:hover{border-color:#b9c4d6;box-shadow:var(--shadow);transform:translateY(-2px)}.deal-tile[hidden]{display:none}.deal-tile-top{display:flex;align-items:center;gap:8px;color:var(--muted);font-size:10px}.deal-tile-top time{margin-left:auto;font-variant-numeric:tabular-nums}.type-badge,.deal-status{padding:4px 8px;border-radius:999px;font-size:9px;font-weight:800;letter-spacing:.04em}.type-badge{background:var(--blue-soft);color:var(--blue-dark)}.type-ma{background:var(--mint-soft);color:var(--mint)}.type-ecm{background:#f2edff;color:#6e45bd}.type-dcm{background:var(--amber-soft);color:var(--amber)}.deal-status{background:var(--surface-soft);border:1px solid var(--line);color:var(--muted)}.deal-tile h3{font-size:17px;line-height:1.35;letter-spacing:-.012em;margin:15px 0 18px;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden}.parties{display:grid;grid-template-columns:1fr 22px 1fr;align-items:center;gap:8px;padding:13px;border-radius:11px;background:var(--surface-soft)}.parties div span,.deal-facts span{display:block;color:var(--muted);font-size:9px;margin-bottom:4px}.parties strong{display:block;font-size:12px}.party-arrow{text-align:center;color:#9aa5b5}.deal-facts{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;padding:15px 0}.deal-facts div+div{border-left:1px solid var(--line);padding-left:12px}.deal-facts strong{font-size:12px;font-variant-numeric:tabular-nums}.deal-tile-foot{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:auto;padding-top:13px;border-top:1px solid var(--line);color:var(--muted);font-size:10px}.deal-tile-foot a{color:var(--blue);text-decoration:none;font-weight:750}.deal-tile-foot a:hover{text-decoration:underline}
