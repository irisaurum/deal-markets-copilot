...
def _deal_bucket_panels(buckets: dict[str, list[dict]]) -> str:
    ...
    for kind, rows in buckets.items():
        ...
        panels.append(f'<section class="bucket-panel" data-bucket-panel="{kind}"{hidden}><div class="bucket-head"><div><h3>{title}</h3><p>{description}</p></div><span>{len(rows)}</span></div><div class="deal-grid">{cards}</div></section>')
    return "".join(panels)
...
def _deal_tiles(rows: list[dict], bucket: str = "deal") -> str:
    ...
    for row in rows:
        ...
        cards.append(f"""<article class="deal-tile" data-deal-bucket="{html.escape(bucket, quote=True)}" data-deal-type="{html.escape(deal_type, quote=True)}" data-deal-search="{html.escape(search, quote=True)}">
...
def _typed_deal_details(row: dict) -> str:
    deal_type = row.get("deal_type")
    if deal_type == "M&A":
        fields = [("Advisors", row.get("advisors")), ("Rationale", row.get("rationale")), ("EV", _deal_amount(row.get("enterprise_value"), row.get("currency", "")))]
    elif deal_type == "DCM":
...
const dealCards=[...document.querySelectorAll('.deal-tile')],dealPanels=[...document.querySelectorAll('.bucket-panel')],dealFilters=[...document.querySelectorAll('.deal-filter')],dealSearch=document.getElementById('deal-search');let activeDeal='deal';
function applyDealFilters(){const q=dealSearch.value.trim().toLowerCase();dealPanels.forEach(panel=>panel.hidden=panel.dataset.bucketPanel!==activeDeal);dealCards.forEach(card=>{const searchOk=!q||card.dataset.dealSearch.includes(q);card.hidden=card.dataset.dealBucket!==activeDeal||!searchOk;});}
