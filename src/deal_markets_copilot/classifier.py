...
CATEGORY_KEYWORDS: dict[str, tuple[str, ...]] = {
    ...
    "ECM": (
        ...
        "размещен акций", "размещение акций", "выпуск акций", "эмиссия акций", "публичн размещен", "допэмисс", "spo", "spо",
...
TECHNICAL_EXCHANGE_NOTICE_PATTERNS = (
    r"^\s*итоги выпуска биржевых облигаций",
    r"^\s*о регистрации (?:дополнительного\s+)?(?:выпуска|проспекта|программы|изменений)",
    r"^\s*о внесении изменений (?:и дополнений )?в .{0,80}(?:решение|проспект|программ|услови).{0,120}(?:выпуск|облигац)",
    r"^\s*(?:об|о) изменении параметров .{0,120}(?:ценн\w* бумаг|облигац|листинг|торг)",
    ...
    r"\bломбардн\w*\s+спис\w*\b",
    r"\b(?:central bank|bank of russia).{0,160}\b(?:collateral eligibility|eligible securities|collateral list)\b",
    r"\b(?:collateral eligibility|eligible securities|collateral list).{0,160}\b(?:central bank|bank of russia)\b",
    ...
    r"^\s*(?:о |информация о )?(?:выплате|выплата).{0,80}купон",
    r"^\s*(?:о |информация о )?погашени\w*.{0,80}облигац",
...
def is_technical_exchange_notice(text: str) -> bool:
    """Recognize exchange plumbing that is traceable but not an economic deal."""
    return any(re.search(pattern, text, re.I) for pattern in TECHNICAL_EXCHANGE_NOTICE_PATTERNS)
