...
Schema-versioned polling state is stored atomically outside tracked artifacts and restored/saved through the GitHub Actions cache. Replay never reads or writes it. Missing state retains deterministic slot gating; corrupted state fails the live run closed before transport.
