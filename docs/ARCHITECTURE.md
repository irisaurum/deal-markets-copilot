...
### 30-minute orchestration and no-op publication

The production schedule is evaluated by one `*/30 * * * *` cron. One injected UTC orchestration clock drives explicit per-source eligibility before transport initialization. Intervals longer than 30 minutes use deterministic UTC slots; disabled, blocked, research, not-due and backoff decisions make zero requests.

Schema-versioned polling state is stored atomically outside tracked artifacts and restored/saved through the GitHub Actions cache. Schema v2 separates the last authoritative `committed` generation, the current runner's `candidate` generation and a narrow `failure_patch`. Discovery may write candidate evidence, but only a verified public no-op or a successful fast-forward bot push promotes it. Verifier, stale-parent, commit and push failures roll back candidate evidence and merge only safe attempt/backoff fields into the prior committed generation. Replay never reads or writes operational state. Missing state retains deterministic slot gating; corrupted or unfinalized cached state fails the live run closed before transport.

Live discovery computes `publish_delta` from exact canonical dataset bytes, stable source-health transitions and a versioned artifact-generator/config fingerprint. When false, the workflow verifies existing artifacts but skips replay/build regeneration, bot commit, push, Pages upload and deployment. See [`CIS_ORCHESTRATOR_IMPLEMENTATION.md`](CIS_ORCHESTRATOR_IMPLEMENTATION.md).
