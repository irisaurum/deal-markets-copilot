...
// SUMMARY
titleBand(summary,"J","DEAL MARKETS COPILOT — SUMMARY",`Quality-controlled transaction monitor | As of ${new Date().toISOString().slice(0,10)} | N/M means insufficient or non-comparable disclosure`);
...
summary.getRange("A7:J7").formulas=[["=COUNTA('Deals'!$A$7:$A$506)","=COUNTIF('Deals'!$C$7:$C$506,\"M&A\")","=COUNTIF('Deals'!$C$7:$C$506,\"ECM\")","=COUNTIF('Deals'!$C$7:$C$506,\"DCM\")","=COUNTIF('Deals'!$AN$7:$AN$506,\"approved\")","=COUNTIF('Deals'!$AN$7:$AN$506,\"review\")","=COUNTA('Financials'!$A$7:$A$506)","=COUNT('Multiples'!$I$7:$I$506)","=COUNT('Multiples'!$J$7:$J$506)","='Sources & QA'!$P$16"]];
...
summary.getRange("A13:D13").formulas=[["=IFERROR(MEDIAN(FILTER('Multiples'!$I$7:$I$506,'Multiples'!$I$7:$I$506>0)),\"N/M\")","=IFERROR(MEDIAN(FILTER('Multiples'!$J$7:$J$506,'Multiples'!$J$7:$J$506>0)),\"N/M\")","=COUNT('Multiples'!$I$7:$I$506)","=COUNT('Multiples'!$J$7:$J$506)"]];
...
titleBand(multiples,"M","MULTIPLES","Formula-driven EV / Revenue and EV / EBITDA; calculations require positive values and matching currencies");
const multHeaders=["Deal ID","Date","Target","Enterprise Value","EV Currency","Revenue LTM","EBITDA LTM","Financials Currency","EV / Revenue","EV / EBITDA","Eligible","Financial Source","Notes"];
multiples.getRange("A6:M6").values=[multHeaders]; headers(multiples.getRange("A6:M6"));
const multMatrix=maRows.map(r=>[r.deal_id,toDate(r.announced_date),r.target_or_issuer,r.enterprise_value,r.currency,r.revenue_ltm,r.ebitda_ltm,r.financials_currency,null,null,null,r.financials_source_name,r.multiple_notes]);
if(multMatrix.length){const end=lastRow(multMatrix.length);multiples.getRangeByIndexes(6,0,multMatrix.length,multHeaders.length).values=multMatrix;for(let row=7;row<=end;row++){multiples.getRange(`I${row}`).formulas=[[`=IFERROR(IF(AND($D${row}>0,$F${row}>0,$E${row}=$H${row}),$D${row}/$F${row},\"\"),\"\")`]];multiples.getRange(`J${row}`).formulas=[[`=IFERROR(IF(AND($D${row}>0,$G${row}>0,$E${row}=$H${row}),$D${row}/$G${row},\"\"),\"\")`]];multiples.getRange(`K${row}`).formulas=[[`=IF(OR($I${row}>0,$J${row}>0),\"YES\",\"NO\")`]];}styleTable(multiples,`A6:M${end}`,"MultiplesTable");multiples.getRange(`A7:H${end}`).format.font={color:C.green,size:9};multiples.getRange(`I7:K${end}`).format.font={color:"#000000",size:9};multiples.getRange(`B7:B${end}`).setNumberFormat("dd-mmm-yyyy");multiples.getRange(`D7:D${end}`).setNumberFormat("#,##0;[Red](#,##0);-");multiples.getRange(`F7:G${end}`).setNumberFormat("#,##0;[Red](#,##0);-");multiples.getRange(`I7:J${end}`).setNumberFormat("0.0x");multiples.getRange(`K7:K${end}`).conditionalFormats.add("containsText",{text:"YES",format:{fill:C.paleGreen,font:{color:"#006100",bold:true}}});multiples.getRange(`K7:K${end}`).conditionalFormats.add("containsText",{text:"NO",format:{fill:C.paleRed,font:{color:C.red,bold:true}}});}
setWidths(multiples,[28,14,24,20,12,18,18,14,14,14,11,32,55],Math.max(maRows.length+7,8)); multiples.freezePanes.freezeRows(6);
...
qa.getRange("K7:K14").values=[["Duplicate deal IDs"],["Missing primary URLs"],["Missing announced dates"],["Unsafe URLs"],["Multiples without EV"],["Multiples with currency mismatch"],["Financials without source"],["Eligible multiple observations"]];
qa.getRange("L7:L14").formulas=[["=SUMPRODUCT(--(COUNTIF('Deals'!$A$7:$A$506,'Deals'!$A$7:$A$506)>1))/2"],["=COUNTIFS('Deals'!$A$7:$A$506,\"<>\",'Deals'!$AS$7:$AS$506,\"\")"],["=COUNTIFS('Deals'!$A$7:$A$506,\"<>\",'Deals'!$B$7:$B$506,\"\")"],["=SUMPRODUCT(--(LEFT('Deals'!$AS$7:$AS$506,4)<>\"http\"),--('Deals'!$A$7:$A$506<>\"\"))"],["=COUNTIFS('Multiples'!$K$7:$K$506,\"YES\",'Multiples'!$D$7:$D$506,\"\")"],["=SUMPRODUCT(--('Multiples'!$K$7:$K$506=\"YES\"),--('Multiples'!$E$7:$E$506<>'Multiples'!$H$7:$H$506))"],["=COUNTIFS('Financials'!$A$7:$A$506,\"<>\",'Financials'!$N$7:$N$506,\"\")"],["=COUNTIF('Multiples'!$K$7:$K$506,\"YES\")"]];
qa.getRange("M7:M14").values=[[0],[0],[0],[0],[0],[0],[0],[">=1"]]; qa.getRange("N7:N13").formulas=Array.from({length:7},(_,i)=>[`=L${7+i}-M${7+i}`]); qa.getRange("O7:O13").values=Array.from({length:7},()=>[0]); qa.getRange("P7:P13").formulas=Array.from({length:7},(_,i)=>[`=IF(ABS(N${7+i})<=O${7+i},\"OK\",\"REVIEW\")`]); qa.getRange("P14").formulas=[["=IF(L14>=1,\"OK\",\"REVIEW\")"]];
qa.getRange("K16:O16").merge();qa.getRange("K16").values=[["Overall model status"]];qa.getRange("P16").formulas=[["=IF(COUNTIF(P7:P14,\"REVIEW\")=0,\"OK\",\"REVIEW\")"]];qa.getRange("K16:P16").format={fill:C.navy,font:{color:C.white,bold:true},borders:{preset:"outside",style:"thin",color:C.border}};qa.getRange("P7:P16").conditionalFormats.add("containsText",{text:"OK",format:{fill:C.paleGreen,font:{color:"#006100",bold:true}}});qa.getRange("P7:P16").conditionalFormats.add("containsText",{text:"REVIEW",format:{fill:C.paleRed,font:{color:C.red,bold:true}}});
setWidths(qa,[28,14,24,30,18,13,50,14,55,3,30,12,12,12,12,14],Math.max(sourceRows.length+7,18)); qa.freezePanes.freezeRows(6);
